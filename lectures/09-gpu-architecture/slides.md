...
Note: This diagram is dense but important. The SM has four partitions, each with a warp scheduler, 32 FP32 cores, 16 FP64 cores, a tensor core, and load/store units. All four partitions share the **register file** (a large SRAM bank holding working variables for ALL resident threads simultaneously, which is what enables zero-cost warp switching) and shared memory / L1 cache. Why block sizes of 128 or 256? Each SM has 4 partitions, each handling warps of 32 threads. A block of 128 = 4 warps = one per partition (clean fit). A block of 256 = 8 warps = two per partition. A block of 100 = 3 full warps + 1 partial warp where 28 of 32 cores sit idle. Always use multiples of 32, and prefer multiples of 128 for best partition alignment. Glossary of SM components: **Dispatch unit** decodes the instruction from the warp scheduler and routes it to the correct execution unit (FP32 cores, FP64 cores, tensor core, or LD/ST). Think of it as the traffic cop between the scheduler and the ALUs. **L0 instruction cache** is a tiny per-partition cache that holds recently fetched instructions. It is NOT the register file. Registers hold data; the L0 I-cache holds the instructions themselves so the warp scheduler doesn't have to fetch from L1 every cycle. **Texture cache** is a read-only cache optimized for 2D/3D spatial locality (used by Tex units and also by `__ldg()` intrinsic for read-only global loads in compute). **Constant cache** is a small broadcast cache for the 64 KB constant memory. When all 32 threads read the same constant address, it serves the entire warp in one cycle. **Tensor core** is a specialized matrix-multiply-accumulate unit that computes a 4x4 matrix multiply in a single cycle (128 FMA operations). We cover tensor cores in depth in Part 6.
